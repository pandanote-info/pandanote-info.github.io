...
&= \left\{
\begin{array}{ll}
\displaystyle\frac{(2m-1)!!}{(2m)!!}\displaystyle{\pi}{2} (n = 2m) \cr
...
&= (n-1)[\int_{0}^{\frac{\pi}{2}} \cos^{n-2}x\,dx - \int_{0}^{\frac{\pi}{2}} \cos^n x\,dx] \nonumber \cr
...
$\{I_n\}$を数列と考えると、(\ref{eq:wallisrec})式は漸化式と考えることができで、かつ2項前の値がわかれば求めることができます。そこで、$\{I_n\}$の最初の2項である$I_0,I_1$を計算すると…
