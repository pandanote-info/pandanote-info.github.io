...
と変換できます。さらに、座標平面上$x軸$正の向きと$\boldsymbol{\mu}$のなす角を$\mu$とすると$\theta=\phi - \mu$となることに留意しつつ、(\ref{eq:probconv})式に(\ref{eq:mnddef})式を適用すると…
...
ここからは、角度($\phi$)以外の数は適当に置き換えつつ計算します。
...
次に$\exp$の指数部の$\phi$とは無関係な部分に着目します。$\frac{r\\|\boldsymbol{\mu}\\|{\sigma^2}} = m$とおくと、
