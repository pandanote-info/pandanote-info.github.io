...
ここで唐突ですが、平均$\boldsymbol{\mu}=(\mu_1,\mu_2)^T$、正定値の分散共分散行列$\Sigma = \sigma^2 I = \left(\begin{array}
\sigma^2 & 0 \cr
0 & \sigma^2
...
&= \frac{1}{2\pi\sigma^2}\exp\left[ -\frac{1}{2}(\boldsymbol{x}-\boldsymbol{\mu})^T\left(\begin{array}
\dfrac{1}{\sigma^2} & 0 \cr
0 & \dfrac{1}{\sigma^2}
\end{array}
